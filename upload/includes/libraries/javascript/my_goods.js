...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        tpl.find('*[item="member_price_1"]').append('<input type="hidden" name="member_price_1['+ item.spec_id +']" value="' + item.member_price_1 + '" />' + item.member_price_1);
        tpl.find('*[item="member_price_2"]').append('<input type="hidden" name="member_price_2['+ item.spec_id +']" value="' + item.member_price_2 + '" />' + item.member_price_2);
        tpl.find('*[item="member_price_3"]').append('<input type="hidden" name="member_price_3['+ item.spec_id +']" value="' + item.member_price_3 + '" />' + item.member_price_3);
        tpl.find('*[item="member_price_4"]').append('<input type="hidden" name="member_price_4['+ item.spec_id +']" value="' + item.member_price_4 + '" />' + item.member_price_4);
        tpl.show();
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
        ...
        tpl.find('*[item="member_price_1"]').val(item.member_price_1);
        tpl.find('*[item="member_price_2"]').val(item.member_price_2);
        tpl.find('*[item="member_price_3"]').val(item.member_price_3);
        tpl.find('*[item="member_price_4"]').val(item.member_price_4);
        tpl.show();
   ...
   $('*[ectype="add_spec_item"]').click(function(){
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
            ...
            var member_price_1=$.trim($(this).find('*[item="member_price_1"]').val());
            var member_price_2=$.trim($(this).find('*[item="member_price_2"]').val());
            var member_price_3=$.trim($(this).find('*[item="member_price_3"]').val());
            var member_price_4=$.trim($(this).find('*[item="member_price_4"]').val());
            ...
            if(isNaN(member_price_1)|| member_price_1<0 || !member_price_1){
                valid && price_error.push(item);
            }
            if(isNaN(member_price_2)|| member_price_2<0 || !member_price_2){
                valid && price_error.push(item);
            }
            if(isNaN(member_price_3)|| member_price_3<0 || !member_price_3){
                valid && price_error.push(item);
            }
            if(isNaN(member_price_4)|| member_price_4<0 || !member_price_4){
                valid && price_error.push(item);
            }
            ...
            item != ';' && SPEC.specs.push({
                ...
                'sku':sku,
                'spec_id':spec_id,
                'member_price_1':number_format(member_price_1, 2),
                'member_price_2':number_format(member_price_2, 2),
                'member_price_3':number_format(member_price_3, 2),
                'member_price_4':number_format(member_price_4, 2)
                });
        });
