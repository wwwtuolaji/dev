...
class TransactionModel extends BaseModel
{
    ...
    function cancel_agent_mod($sn, $goods_count, $user_id)
    {
        ...
        if ($sn['transaction_status'] == 1) {
            ...
            $transaction_price = (int)(string)($transaction_price_total * 100) + (int)(string)($sn['univalent'] * 100) * $sn['transaction_count'];
            ...
            $temp = (int)(string)($_price * 100) + (int)(string)($sn['waiting_pay_price'] * 100);
            $cur_price = $temp / 100;
            ...
            if ($result_re) {
                ...
                if (!$have_add) {
                    ...
                    //2.计算减少后的金额
                    $set_admin_money = ((int)(string)(100 * $admin_money) -(int)(string)(100 * $sn['waiting_pay_price'])) / 100;
                    //3.编辑金额
                    ...
                    $add_admin_array = array(
                        'transaction_sn' => $sn['transaction_sn'],//充值id
                        'money_from' => 0,//0预存款，1支付宝, 2微信
                        'transaction_type' => 1,//0,收入 1支出
                        'receive_money' => $sn['waiting_pay_price'],//收入或减去的金额
                        'pay_time' => time(),//支付时间
                        'platform_from' => 2,//0,茶通历史表，1，商城 2，transaction3,个人充值
                        'use_money_history' => $set_admin_money,//当前的金额
                        'user_id' => 1,//user_id
                        'comments' => "用户({$member_info['user_name']})撤销购买商品(id={$sn['goods_id']}),退款总价是({$sn['waiting_pay_price']})元",//备注
                    ); 
            ...
            if ($result_log && $result && $result_re && $result_add_admin && $result_member_admin) {
                $this->db->query("COMMIT");
                $this->db->query("SET AUTOCOMMIT=1");
                return true;
            } else {
    ...
    function produce_buy_deal($buy_arr, $tran)
    {
        ...
        foreach ($buy_arr as $buy) {
            ...
            $member_info_buy = $member_mod->get($buy['user_id']);
            $member_info_sell = $member_mod->get($tran['user_id']);
            //买的数量大于 个人要卖的数量直接成交
            if ($buy['transaction_count'] >= $count) {
                ...
                $waiting_pay_price = (int)(string)($buy['waiting_pay_price'] * 100) - (int)(string)($buy['transaction_price'] * 100) * $count;
                ...
                //获取当前的waiting_price               
                $temp = (int)(string)($_price * 100) + (int)(string)($buy['waiting_pay_price'] * 100);
                $cur_price = $temp / 100;
                ...
                if ($pay_result) {
                    ...
                    if (!$have_add) {
                        ...
                        $admin_money = $member_mod->get_money(1);
                        //4.2 计算增加后的金额
                        $set_admin_money = ((int)(string)(100 * $admin_money) - (int)(string)($buy['waiting_pay_price'] * 100)) / 100;
                        //4.3 编辑金额
                        $edit_admin_array = array('use_money' => $set_admin_money);
                        $result_member_admin = $member_mod->edit(1, $edit_admin_array);
                        //4.4 增加admin的金额日志记录
                        $add_admin_array = array(
                            'transaction_sn' => $result,//充值id
                            'money_from' => 0,//0预存款，1支付宝, 2微信
                            'transaction_type' => 1,//0,收入 1支出
                            'receive_money' => $buy['waiting_pay_price'],//收入或减去的金额
                            'pay_time' => time(),//支付时间
                            'platform_from' => 2,//0,茶通历史表，1，商城 2，transaction3,个人充值
                            'use_money_history' => $set_admin_money,//当前的金额
                            'user_id' => 1,//user_id
                            'comments' => "用户({$member_info_buy['user_name']})一次性买入({$member_info_sell['user_name']})的商品（id={$tran['goods_id']}）,总价是({$buy['waiting_pay_price']})元."//备注
                        );
                        $result_add_admin = $money_hitory_mod->add($add_admin_array);
                ...
                if ($add_log && $result && $buy_edit && $buy_insert && $sell_edit && $sell_later && $pay_result && $result_add_admin &&$result_member_admin) {
                    $this->db->query('COMMIT');
                    $this->db->query("SET AUTOCOMMIT=1");
                    return true;
                } else {
            ...
            $cur_price = "select use_money from ecm_member where user_id='{$tran['user_id']}'";
            $_price = $this->getone($cur_price);
            $price_temp = (int)(string)($_price * 100);

            $temp_add_price = $buy['transaction_count'] * (int)(string)($buy['transaction_price'] * 100);
            ...
            if ($result_re) {
                ...
                if (!$have_add) {
                    ...
                    $admin_money = $member_mod->get_money(1);
                    //4.2 计算增加后的金额
                    $set_admin_money = ((int)(string)(100 * $admin_money) - (int)(string)($add_price * 100)) / 100;
                    //4.3 编辑金额
                    $edit_admin_array = array('use_money' => $set_admin_money);
                    $result_member_admin = $member_mod->edit(1, $edit_admin_array);
                    //4.4 增加admin的金额日志记录
                    $add_admin_array = array(
                        'transaction_sn' => $result_re,//充值id
                        'money_from' => 0,//0预存款，1支付宝, 2微信
                        'transaction_type' => 1,//0,收入 1支出
                        'receive_money' => $add_price,//收入或减去的金额
                        'pay_time' => time(),//支付时间
                        'platform_from' => 2,//0,茶通历史表，1，商城 2，transaction3,个人充值
                        'use_money_history' => $set_admin_money,//当前的金额
                        'user_id' => 1,//user_id
                        'comments' => "用户({$member_info_buy['user_name']})部分买入({$member_info_sell['user_name']})的商品（id={$tran['goods_id']}）,单价是({$buy['transaction_price']})元.数量是（{$buy['transaction_count']}）"//备注
                    );
                    $result_add_admin = $money_hitory_mod->add($add_admin_array);
            ...
            if ($add_log && $result_re && $buy_edit_re && $buy_insert_re && $sell_edit_re && $up_usemoney &&$result_add_admin && $result_member_admin) {
                $this->db->query('COMMIT');
            } else {
                $this->db->query('ROLLBACK');
                echo "交易失败";
        ...
        function produce_sell_deal($sell_arr, $tran)
        {
            ...
            foreach ($sell_arr as $key => $sell) {
                ...
                $member_info_sell = $member_mod->get($sell['user_id']);
                $member_info_buy = $member_mod->get($tran['user_id']);
                //卖的数量大于买的数量一单成交
                if ($sell['transaction_count'] >= $count) {
                    ...
                    $_price = $this->getone($cur_price);
                    $temp = (int)(string)($_price * 100) + (int)(string)($tran['waiting_pay_price'] * 100);
                    $cur_price = $temp / 100;
                    ...
                    if ($result) {
                        ...
                        if (!$have_add) {
                            ...
                            //获取
                            //交易成交，需要将买方用户购买商品前存到admin用户的钱移出，转给卖方
                            //4.1 获取到admin账户的金额
                            $admin_money = $member_mod->get_money(1);
                            //4.2 计算减少后的金额
                            $set_admin_money = ((int)(string)(100 * $admin_money) - (int)(string)($tran['waiting_pay_price'] * 100)) / 100;
                            //4.3 编辑金额
                            $edit_admin_array = array('use_money' => $set_admin_money);
                            $result_member_admin = $member_mod->edit(1, $edit_admin_array);
                            //4.4.1
                            ...
                            $add_admin_array = array(
                                ...
                                'money_from' => 0,//0预存款，1支付宝, 2微信
                                'transaction_type' => 1,//0,收入 1支出
                                'receive_money' => $tran['waiting_pay_price'],//收入或减去的金额
                                'pay_time' => time(),//支付时间
                                'platform_from' => 2,//0,茶通历史表，1，商城 2，transaction3,个人充值
                                'use_money_history' => $set_admin_money,//当前的金额
                                'user_id' => 1,//user_id
                                'comments' => "用户({$member_info_buy['user_name']})一次性买入({$member_info_sell['user_name']})的商品（id={$tran['goods_id']}）,总价是({$tran['waiting_pay_price']})元."//备注
                            );
                            $result_add_admin = $money_hitory_mod->add($add_admin_array);
                        } else {
                            echo "重复添加金额，非法操作04";
                    ...
                    if ($add_have && $result && $buy_edit && $sell_edit && $sell_alter && $buy_insert && $pay_result && $result_add_admin && $result_member_admin) {
                        $this->db->query('COMMIT');
                        $this->db->query("SET AUTOCOMMIT=1");
                        echo "请求成功";
                        return true;
                ...
                $temp_re = (int)(string)($_price_re * 100);
                $add_price_temp = (int)(string)($sell['transaction_price'] * 100);
                ...
                //修改买方的状态 ？有问题 这个应该是买的数量减去卖的数量
                //修改1.获取到实时的transaction 数量
                $buy_cur_transaction_info = $transaction_mod->get($tran['transaction_sn']);
                //修改2 用当前实时的金额减去卖出的金额
                $buy_data_re['transaction_count'] = $buy_cur_transaction_info['transaction_count'] - $sell['transaction_count'];
                /*原来代码 $buy_data_re['transaction_count'] = $sell['transaction_count'];*/
                $buy_data_re['have_transaction'] = 1;
                $buy_temp_re = (int)(string)($tran['waiting_pay_price'] * 100);
                ...
                if ($result_re) {
                    ...
                    if ($have_add) {
                        ...
                        $admin_money = $member_mod->get_money(1);
                        //4.2 计算增加后的金额
                        $set_admin_money = ((int)(string)(100 * $admin_money) - (int)(string)($add_price)) / 100;
                        //4.3 编辑金额
                        $edit_admin_array = array('use_money' => $set_admin_money);
                        $result_member_admin = $member_mod->edit(1, $edit_admin_array);
                        //4.4 增加admin的金额日志记录
                        $add_admin_array = array(
                            ...
                            'money_from' => 0,//0预存款，1支付宝, 2微信
                            'transaction_type' => 1,//0,收入 1支出
                            'receive_money' => $add_price / 100,//收入或减去的金额
                            'pay_time' => time(),//支付时间
                            'platform_from' => 2,//0,茶通历史表，1，商城 2，transaction3,个人充值
                            'use_money_history' => $set_admin_money,//当前的金额
                            'user_id' => 1,//user_id
                            'comments' => "用户({$member_info_buy['user_name']})部分买入({$member_info_sell['user_name']})的商品（id={$tran['goods_id']}）,单价是({$tran['transaction_price']})元.数量是（{$sell['transaction_count']}）"//备注
                        );
                        $result_add_admin = $money_hitory_mod->add($add_admin_array);
                ...
                if ($add_log && $result_re && $sell_edit_re && $sell_alter_re && $buy_edit_re && $buy_insert_re && $up_price_re && $result_add_admin && $result_member_admin) {
                    $this->db->query('COMMIT');
                    $this->db->query("SET AUTOCOMMIT=1");
                } else {
                    $this->db->query('ROLLBACK');
        ...
        function insert_warehouse($user_id, $goods_id, $count, $price)
        {
            ...
            if ($warehouse['warehouse_id']) {
                $goods_count = $warehouse['goods_count'] + $count;
                $transaction_price = (int)(string)(($warehouse['transaction_price']) * 100) + (int)(string)($price * 100) * $count;
                $transaction_price = $transaction_price / 100;
            ...
            $own_data['transaction_price'] = (int)(string)($price * 100) * $count / 100;
        ...
        /*获取更为详细的交易信息*/
        function get_transaction($sn,$where=''){
            $sql  = "select a.*,b.user_name from ecm_transaction as a inner join ecm_member as b on a.user_id = b.user_id where transaction_sn = '$sn'$where";
