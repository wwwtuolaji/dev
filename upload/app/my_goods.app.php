        ...
        switch ($goods['spec_qty'])
        {
            case 0: // 没有规格
                $specs[intval($_POST['spec_id'])] = array(
                    ...
                    'member_price_1'=>$this->_filter_price($_POST['member_price_1']),
                    'member_price_2'=>$this->_filter_price($_POST['member_price_2']),
                    'member_price_3'=>$this->_filter_price($_POST['member_price_3']),
                    'member_price_4'=>$this->_filter_price($_POST['member_price_4']),
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
                                ...
                                'member_price_1'=>$this->_filter_price($_POST['member_price_1'][$key]),
                                'member_price_2'=>$this->_filter_price($_POST['member_price_2'][$key]),
        ...
        foreach ($data['specs'] as $key => $spec)
        {
            ...
            if (empty($default_spec))
            {
                $default_spec = array(
                    ...
                    'price' => $spec['price'],
                    'member_price_1'=>$spec['member_price_1'],
                    'member_price_2'=>$spec['member_price_2'],
                    'member_price_3'=>$spec['member_price_3'],
                    'member_price_4'=>$spec['member_price_4'],
                    );
            }
        }
