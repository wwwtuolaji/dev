...
class CartApp extends MallbaseApp
{
    ...
    function add()
    {
        ...
        if (!$this->visitor->has_login)
        {
            ...
            $spec_model =& m('goodsspec');
            $spec_info  =  $spec_model->get(array(
                'fields'        => 'g.store_id, g.goods_id, g.goods_name, g.spec_name_1, g.spec_name_2, g.default_image, gs.spec_1, gs.spec_2, gs.stock, gs.price, gs.member_price_1, gs.member_price_2,gs.member_price_3, gs.member_price_4',
                'conditions'    => $spec_id,
                'join'          => 'belongs_to_goods',
            ));
            //判断会员等级，根据会员等级分配会员价格
            $member_info=$this->visitor->get();
            
            if ($member_info['member_level']==1) {
                $spec_info['price']=$spec_info['member_price_1'];
            }else if ($member_info['member_level']==2) {
                $spec_info['price']=$spec_info['member_price_2'];
            }else if ($member_info['member_level']==3) {
                $spec_info['price']=$spec_info['member_price_3'];
            }else if ($member_info['member_level']==4) {
                $spec_info['price']=$spec_info['member_price_4'];
            }
        }
        if (!$spec_info)
        {
